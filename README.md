...
| **C4** | Trajectory-level eval catches config changes that keep every final answer correct while the path degrades. Endpoint-level eval passes them. | 15 candidate changes (model swaps, prompt edits, tool-doc and step-budget cuts) × the 29 incidents with a reviewed golden trajectory, each replayed against the same deterministic tools. A change is scored only when every one of its 29 runs replays. | 15 candidate changes on 29 tasks → endpoint eval missed **2**, trajectory eval caught **2**. 310 task runs kept a correct answer on a worse path. |
...
**Status.** Both claims are measured and every number in the table above came from a run on this machine. 29 of the 30 incidents have a golden trajectory: each one a baseline run that was printed and accepted on its own. `incident_log_flood` has none — the baseline spent eleven of its sixteen steps on nine differently worded log searches that all returned the same four lines, and ran out of budget before it acted; its recording is kept so the run can be read back. All 15 candidate changes are recorded on all 29 of those incidents.
