...
|---|---|---|---|
| **C1** | Crash the agent in any of the four commit windows and a durable resume reproduces the crash-free run with no repeated side effect. A from-scratch baseline repeats them. | Every crash point once: each of the four windows × each tool call of the reference run. Each trial resumes a durable run and a naive run from the same crash point, then compares the final answer and the side-effect ledger against a crash-free reference. Exhaustive rather than sampled, so the result is the same on every machine. | 60 crashes — every W1–W4 × tool-call point once, no sampling → **100.0%** correct resume, **0** duplicate side effects. Naive baseline: **52.9%** of the 34 crashes that landed after a side effect repeated one (44 extra actions; 30.0% of all 60). |
| **C4** | Trajectory-level eval catches config changes that keep every final answer correct while the path degrades. Endpoint-level eval passes them. | 15 candidate changes (model swaps, prompt edits, tool-doc and step-budget cuts) × the 29 incidents with a reviewed golden trajectory, each replayed against the same deterministic tools. A change is scored only when every one of its 29 runs replays; the result cell names how many made it. | 13 of 15 candidate changes on 29 tasks → endpoint eval missed **2**, trajectory eval caught **2**. 263 task runs kept a correct answer on a worse path. |
...
**Status.** C1 is complete. 29 of the 30 incidents have a golden trajectory: each one a baseline run that was printed and accepted on its own. `incident_log_flood` has none — the baseline spent eleven of its sixteen steps on nine differently worded log searches that all returned the same four lines, and ran out of budget before it acted. Of the 15 candidate changes, 6 are measured; the other 9 stopped mid-recording when the API credit ran out, and a config whose recording stops before its run does is skipped rather than scored on half a task set. `npm run record -- --candidates` resumes where it stopped — every response already recorded is reused, so only the missing turns are paid for.
...
- **A golden path is what a competent baseline did, not the ideal path.** Several contain an action that could not have helped — restarting a service whose failure is a revoked credential or a missing font, then verifying and escalating. That is ordinary on-call behaviour, and editing it out would make the reference a hand-written ideal, which is the one thing a golden must not be. It does mean a candidate that skips the futile attempt is scored as a deviation.
- **Log search is easier here than in production.** Each incident carries four to six log lines and the tool returns up to six matches, so any query that matches at all returns most of the log. Investigation is correspondingly cheap. Every config faces the same tool, so the comparison between them is unaffected, but the absolute number of search calls in a path means less than it would against a real log store.
